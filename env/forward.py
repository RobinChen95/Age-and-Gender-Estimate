...
def block35(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    with tf.variable_scope(scope, 'Block35', [net], reuse=reuse):
        ...
        # 分支3：1x1 conv -> 3x3 conv  -> 3x3 conv (相当于 9x9 conv )
...
def block17(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block17', [net], reuse=reuse):
        ...
        # 分支2：1x1 conv -> 1x7 conv  -> 7x1 conv (相当于 7x7 conv )
        with tf.variable_scope('Branch_1'):
...
def block8(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.variable_scope(scope, 'Block8', [net], reuse=reuse):
        ...
        # 分支2：1x1 conv -> 1x3 conv -> 3x1 conv (相当于 3x3 conv )
        with tf.variable_scope('Branch_1'):
...
def reduction_a(net, k, l, m, n):
    ...
    with tf.variable_scope('Branch_0'):
        tower_conv = slim.conv2d(net, n, 3, stride=2, padding='VALID',
                                 scope='Conv2d_1a_3x3')
    # 分支2：3x3 conv -> 3x3 conv (相当于 9x9 conv )
    with tf.variable_scope('Branch_1'):
...
def forward(images, keep_probability, phase_train=True, bottleneck_layer_size=128, weight_decay=0.0, reuse=None):
    net, end_points = _inference(images=images, keep_probability=keep_probability, phase_train=phase_train,
                                 bottleneck_layer_size=bottleneck_layer_size, weight_decay=weight_decay, reuse=reuse)
    return _inception_resnet_v1_out(net, end_points)
